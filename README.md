...
- **Multimodal input needs its own data-integrity checks, not just the text ones.** A photo of a recipe card written in "cups" was silently stored as the same number of *kilograms* — the tool had no unit field to catch the mismatch. The fix wasn't just a prompt tweak: `add_recipe` now takes an explicit `unit` per ingredient and hard-blocks on any mismatch against the tracked unit, so the model can't silently misconvert a quantity it read from an image.
